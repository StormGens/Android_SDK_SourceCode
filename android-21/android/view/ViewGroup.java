...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (onFilterTouchEventForSecurity(ev)) {
            ...
            if (actionMasked == MotionEvent.ACTION_DOWN) {
                ...
                //如果开始一个新的触摸手势，抛弃所有之前的状态
                //因为Framework会在程序切换、ANR等其他状态变更时候丢掉up或cancel事件。
                cancelAndClearTouchTargets(ev);//DOWN是后续事件的起点，一旦收到DOWN，先清除以前的所有状态
                resetTouchState();
            }
            
            /* 【【【Step 2】】】 检查interception的情况 */
            ...
            if (actionMasked == MotionEvent.ACTION_DOWN
                ...
                if (!disallowIntercept) {//如果允许拦截
                    intercepted = onInterceptTouchEvent(ev);//这里判断是否拦截点击事件，如果拦截，则intercepted=true  
                    ev.setAction(action); // restore action in case it was changed
                    //【汉】恢复Action因为上面改变了它
                } else {//不允许拦截的话，直接置为false
                    intercepted = false;
                }
            } else {
                ...
                //没有touchTarget并且该Action不是一个最原始的down事件，所以该view group继续拦截触摸事件。
                intercepted = true;
            }
            ...
            if (mFirstTouchTarget == null) {
                ...
                //没有触摸的目标，所以以普通的view来对待这里就是ViewGroup对点击事件的处理
    ...
    /**
     * 实现该方法来拦截所有的屏幕触摸事件，这将允许你来关注所有派遣到你的子view的事件。
     * 并且拥有所有点的动作手势。
     * 
     * <p>因为该方法与View的{@link View#onTouchEvent(MotionEvent)方法有相当复杂的互动，
     * 所以，使用该方法时候需要多加小心。
     * 
     * 并且使用的时候两个方法都要被以正确的途径实现，事件将以以下顺序被接收：
     * <ol>
     * <li> 你将在这里得到触摸的按下（down）事件
     * <li> 得到的按下（down）事件要么通过这个视图组的子处理，或者给自己的onTouchEvent（）方法来处理;
     * 这意味着你要实现的onTouchEvent（）方法并返回true。
     * 这样你会继续看到手势的其余部分（而不是寻找一个父视图来处理它）
     *  。此外，通过从的onTouchEvent（）返回的true，则不会收到任何onInterceptTouchEvent之后的触摸事件（），
     * 所有触摸的处理必须的像正常情况一样在onTouchEvent（）里面实现。
     * <li>如果你将该方法返回false，所有跟着的事件（上层直至最顶层）都将从这里开始传递，最终传递至目标的onTouchEvent()
     * <li>如果你从这里返回true，您将不会收到任何下列事件：目标视图将收到除ACTION_CANCEL之外和该事件相同的事件，
     *  并且所有事件将被发送到您的onTouchEvent（）方法，不再出现在这里。
     *
     * * * * * * * * * * * * * * * * * * * * * * *
     ...
     * </ol>
     *
     * @param ev The motion event being dispatched down the hierarchy.【汉】运动事件分派下来的层次结构。
     * @return Return true to steal motion events from the children and have
     * them dispatched to this ViewGroup through onTouchEvent().
     * The current target will receive an ACTION_CANCEL event, and no further
     * messages will be delivered here.
     * 
     * @return 返回true来窃取children的运动事件，让他们分派到这个ViewGroup的onTouchEvent（）方法中。
     *  目前的目标将收到ACTION_CANCEL事件，并没有进一步的消息将被送到这里。
     */
