...
/**
 * 
 * View视图层级的最顶级，实现了View和WindManager之间必要的协议。
 * 这大部分是补足{@link WindowManagerGlobal}的更多实现细节
 * * * * *
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
    ...
    /**
     *【汉】一个输入事件会被分为多阶段，形成一个处理的责任链，该类为实现其中每一个阶段的基类。
     * <p>
     * 输入事件会从{@link #deliver}方法传过来，该Stage获得输入事件后，可以选择结束或者传递到下一个
     * Stage
     * 
     * 
     * 
    ...
    abstract class InputStage {
        ...
        protected static final int FORWARD = 0;//向后传递
        protected static final int FINISH_HANDLED = 1;//结束————已处理
        protected static final int FINISH_NOT_HANDLED = 2;//结束————不处理
        ...
        /**
         * 将input event标记为已结束，然后传递到下一个stage
         * * *
        ...
        /**
         * 把{@link #onProcess} 得到的结果应用到指定的event上。
         * * *
        ...
        protected void onDeliverToNext(QueuedInputEvent q) {
            ...
            if (mNext != null) {//如果下一个事件处理器不为空，则让下一个事件处理器处理
                mNext.deliver(q);
            } else {//所有的都处理器都完成了处理，调用finish告知server端事件已经被处理
                finishInputEvent(q);
            }
        }
     ...
     * 
     * 实现一个输入管道阶段的基类,支持异步和无序的输入事件的处理。
     * <p>除了可以完成所有普通的InputStage可以做的外，AsyncInputStage还可以推迟提送过来的input event并且
     * 在以后再处理（finish或者forward）。 </>
     * * *
     * * * *
     * <p>
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        protected int onProcess(QueuedInputEvent q) {
            if (q.mEvent instanceof KeyEvent) {
                ...
                //如果在传递一个非按键event，保证window当前允许开始更新
                
                // If delivering a new non-key event, make sure the window is
                // now allowed to start updating.
                handleDispatchDoneAnimating();
                final int source = q.mEvent.getSource();
                if ((source & InputDevice.SOURCE_CLASS_POINTER) != 0) {//如果是pointer
                    return processPointerEvent(q);
                } else if ((source & InputDevice.SOURCE_CLASS_TRACKBALL) != 0) {//如果是轨迹球
         ...
         */
        private int processPointerEvent(QueuedInputEvent q) {
            final MotionEvent event = (MotionEvent)q.mEvent;//转化为MotionEvent

            mAttachInfo.mUnbufferedDispatchRequested = false;
            boolean handled = mView.dispatchPointerEvent(event);//将MotionEvent派遣到mView中处理
