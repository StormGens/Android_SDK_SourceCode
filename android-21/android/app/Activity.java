...
public class Activity extends ContextThemeWrapper
    ...
    /**
     * 【汉】当一个点击屏幕时间没有被任何位于那个位置的view处理，调用这个方法。
     *该方法最主要是用于用户点击了的window的边界之外位置，没有view可以接收该点击时间时候的的处理。
     * 
     * 
     * * * * * *
     ...
     *
     * @param event The touch screen event being processed.被处理的触摸屏事件
     *
     * @return Return true if you have consumed the event, false if you haven't.
     * The default implementation always returns false.
     * 如果你消费（处理）了该点击事件，返回TRUE，否则返回false，默认的实现都是返回false*
     */
    ...
    /**
     * 【汉】当键盘任意键被点击、屏幕被触摸、轨迹球事件发送到Activity，都会调用该方法。
     * 如果你想知道用户在该Activity以何种方式与该设备进行交互，你的Activity需要运行实现此方法。
     * * *
     * 该方法和 {@link #onUserLeaveHint}的意图是用来帮助Activity智能管理状态栏通知。
     * 具体而言，是为了帮助Activity 确定适当的时间去取消一个通知。
     * 
     * <p>所有对Activity {@link #onUserLeaveHint} 回调方法的调用都是伴随着调用{@link #onUserInteraction}方法
     * 这将确保相关用户活动告知您的Activity，如拉下通知面板和在那里触摸的项目。
     * 
     * * *
     *<p>请注意，此回调将用于触摸手势的按下(down)动作来调用，但可能不被用于触摸手势的抬起(up)动作操作调用。
     * 
     * 
     * * * * * * * *
     ...
     *
     * 【汉】被调用来处理触摸屏事件。您可以覆盖这个拦截所有触摸屏事件，他们被分派到了窗口前。一定要调用这个实现确保
     * 触摸屏事件被正常处理。
     * * *
     * @param ev The touch screen event.触摸屏幕事件
     *
     * @return boolean Return true if this event was consumed. 当触摸事件被处理时候返回true
     */
