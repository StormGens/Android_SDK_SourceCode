...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
     ...
     * 
     * 把触摸屏事件向下分发到目标View，如果这个View就是目标，就到此为止。* *
     *
     ...
     * * * 
     *
     * @param event 需要用过滤的动作事件 The motion event to be filtered.
     * @return True if the event should be dispatched, false if the event should be dropped.
     *          当该事件应该被派遣时返回true，当该事件应该被抛弃掉时候返回false*
     *
     * @see #getFilterTouchesWhenObscured
     */
    ...
    public final boolean dispatchPointerEvent(MotionEvent event) {
        if (event.isTouchEvent()) {// 如果是Touch事件
    ...
    /**
     * 
     * 【注】实现onTouchEvent方法可以处理View的触屏事件，返回true表明事件已被处理，不需要继续派发，否则返回false。
     * *
     * <p> （该方法一般不需要重写，但在重写时候请注意：）如果该方法要用来检测Click行为，建议把这些事件的实现放到
     * {@link #performClick()}中来，在这里只需要调用它。这样就可以确保系统的统一性。
     * 包括以下几种：
     <ul>
     * <li>根据手机设置，点击屏幕时候可以发出声。
     * <li>调用 OnClickListener 方法
     * <li>当辅助功能开启时可以处理 {@link AccessibilityNodeInfo#ACTION_CLICK ACTION_CLICK} 事件
     * </ul>
     * *
     * * * * 
     ...
     *
     * @param event The motion event.
     * @return True if the event was handled, false otherwise.返回true表明事件已被处理，不需要继续派发，否则返回false。
     */
